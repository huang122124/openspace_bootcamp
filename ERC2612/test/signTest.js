...
describe("对多个地址用户进行签名，在合约中进行验签", function () {
  it("verify sign", async function () {
    ...
    const nft_market = await ethers.deployContract("Permit_NftMarketplace", ['0xf0f93144CECa5F5bbE6B953bDC3dD4991c2Ab7d3', '0x9bE7bbB4659109E56EdC7637A5619DCB5B9a43bF'])
    ...
    // 对消息进行签名
    const salt = 888
    const msg_hash = ethers.solidityPackedKeccak256(["address","uint256"], [ addr1.address,salt]);
    const sign = await signer.signMessage(ethers.getBytes(msg_hash));
    console.log("msg_hash:"+msg_hash);
    //调用合约进行验证
    let verified = await nft_market.verify_signature(addr1.address,salt,sign)
